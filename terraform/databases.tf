...
locals {
  ...
  databases = {
    postgres = {
      ...
      backup = ["/bin/bash", "-c", "export PGPASSWORD=${random_password.database_password.result}; pg_dumpall -U ${random_string.database_username.result} -h $DATABASE_HOST -p $DATABASE_PORT -Fc > ${local.backup_location}"]
    }
    mysql = {
    ...
    mariadb = {
      ...
      port = 3306
      live_check = ["/bin/bash","-c", "mysql -u root -p${random_password.database_password.result} -e 'SELECT 1'"]
      backup = ["/bin/bash","-c", "mysqldump -u root -p${random_password.database_password.result} --all-databases --skip-lock-tables > ${local.backup_location}"]
    }
  }
}
